...
function infoConfirm() {
	...
	/*
	 * // http://ibulkamgodoldol.tistory.com/20 var UserPassword =
	 * document.reg_frm.pw; if (UserPassword.value.length < 8) { alert("비밀번호는
	 * 영문(대소문자구분),숫자,특수문자(~!@#$%^&*()-_? 만 허용)를 혼용하여 8~16자를 입력해주세요."); return
	 * false; } if (!UserPassword.value
	 * .match(/([a-zA-Z0-9].*[!,@,#,$,%,^,&,*,?,_,~,-])|([!,@,#,$,%,^,&,*,?,_,~,-].*[a-zA-Z0-9])/)) {
	 * alert("비밀번호는 영문(대소문자구분),숫자,특수문자(~!@#$%^&*()-_? 만 허용)를 혼용하여 8~16자를
	 * 입력해주세요."); return false; }
	 */
	/*
	 * if (document.reg_frm.phone.value.substr(0,3) != "010") {
	 * alert(document.reg_frm.phone.value.substr(0,3)); alert("올바른 휴대전화 번호를
	 * 입력해주세요"); reg_frm.id.focus(); return; }
	 * 
	 * if (document.reg_frm.phone.value.length != 11){ alert("휴대전화 번호 11 자리를
	 * 입력해주세요"); reg_frm.id.focus(); return; }
	 */
	...
	if (check) {
		...
		alert("취소되었습니다.");
	// console.log(SHA256("Test").toUpperCase()) ;
}

// 참고사이트 : http://fruitdev.tistory.com/191
// 확인용 : http://www.convertstring.com/ko/Hash/SHA256
// 512 : http://aljjabaegi.tistory.com/338
function SHA256(s) {
