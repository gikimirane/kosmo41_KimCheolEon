...
<%
	...
	if(session.getAttribute("checking").equals("Yes")) {
		checking = true;
	}
%>
...
<style type="text/css">body{background-color:#0;background-repeat:repeat;}</style><main id="content" class="event_view" data-event-no="156849">
	...
	function join_join(){
		   $.ajax({
		      url : 'join_join',
		      type : 'POST',
		      data : 'name=' + scriptUsername + "&numId=" +  ${join_view.numId} + "&fixed=" + ${join_view.fixed} +  "&count=" + ${joinapplycount.count} + "&userEmail=" + '<%=userEmail%>',
		      dataType : 'json',
